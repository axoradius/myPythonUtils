...
def endExecution(startTime=""):
    ...
    currT = datetime.datetime.now()
    if isinstance(startTime, (datetime.datetime,datetime.date)): #this ensures the provided value has a valid datetime stamp or a valid date stamp
        durationSeconds = (currT - startTime).total_seconds() #this calculation returns an object of type "timedelta". That type has methods such as total_seconds() 
...
def convertToGif(vidFolderPath, srcName, targetExt):
    ...
    print(f"base name is {srcNameBase} with ext {srcNameExt} with size {srcSize} --> {targetName}")

    # video conversion to gif
    if os.path.exists(targetFullPath):
        print(f"skipping confersion to {targetExt} because {targetName} exists already")
    else:
